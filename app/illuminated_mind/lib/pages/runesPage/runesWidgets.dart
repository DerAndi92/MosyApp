...
class _RuneSectionSate extends State<RuneSection> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return ScopedModelDescendant<QuestModel>(
      builder: (context, child, model) => Positioned(
        top: this.widget.position[0] + ((MediaQuery.of(context).size.height / 2 - this.widget.position[0] - ((height * (1 + animation.value)) / 2)) * animation.value),
        left: this.widget.position[1] + ((MediaQuery.of(context).size.width / 2 - this.widget.position[1] - ((width * (1 + animation.value)) / 2)) * animation.value),
        child: Container(
          height: height * (1 + animation.value),
          width: width * (1 + animation.value),
          child:
            model.isRuneUsed(this.widget.color) ? Container() :
          Opacity(
